...
- The PDF scanner now keeps qpdf's output when qpdf exits with warnings instead of discarding it. qpdf warns about a large share of real-world PDFs — a cross-reference table it reconstructed, a page tree it repaired — while still writing a faithful expansion, so discarding it left compressed object streams unexamined on those files.

## [10.4.1]

### Fixed

- Entries progress bar broken DOM

## [10.4.0]

### Added

- New `esFormsEntryProgress` shortcode.
...
- Re-added missing `$additionalClass` passthrough to the `divider` component.

### Changed

- Updated admin notification style.
...
- Updated JS dependencies: `choices.js` to `^11.2.1`, `caniuse-lite` to `^1.0.30001780`, `baseline-browser-mapping` to `^2.10.10`, and `lint-staged` to `^16.4.0`.
...
- Multistep form now properly resets the steps and scrolls to the first step.
...
- Moments integration fix for empty form fields.
