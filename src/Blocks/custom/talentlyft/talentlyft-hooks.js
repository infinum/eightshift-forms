...
export const hooks = () => {
	const { blockName } = select(STORE_NAME).getBlock('talentlyft');
		...
		if (name === `${namespace}/${blockName}`) {
			if (typeof esFormsLocalization !== 'undefined' && isArray(esFormsLocalization?.additionalBlocks)) {
				esFormsLocalization.additionalBlocks.forEach((element) => {
					if (!settings.attributes.talentlyftAllowedBlocks.default.includes(element)) {
						settings.attributes.talentlyftAllowedBlocks.default.push(element);
					}
				});
			}
