...
$workableFormServerSideRender = Components::checkAttr('workableFormServerSideRender', $attributes, $manifest);
$workableFormPostId = Components::checkAttr('workableFormPostId', $attributes, $manifest);
$workableFormDataTypeSelector = Components::checkAttr('workableFormDataTypeSelector', $attributes, $manifest);

// Check if Workable data is set and valid.
$isSettingsValid = apply_filters(SettingsWorkable::FILTER_SETTINGS_IS_VALID_NAME, $workableFormPostId);

$workableClass = Components::classnames([
	Components::selector($blockClass, $blockClass),
	Components::selector(!$isSettingsValid, $invalidClass),
]);

// Bailout if settings are not ok but show msg only in editor.
if (!$isSettingsValid && $workableFormServerSideRender) {
	?>
		<div class="<?php echo esc_attr($workableClass); ?>">
			...
			<br />
			<b><?php esc_html_e('The Workable integration is not configured correctly.', 'eightshift-forms'); ?></b>
			<br />
...
echo apply_filters( // phpcs:ignore Eightshift.Security.ComponentsEscape.OutputNotEscaped
	Workable::FILTER_MAPPER_NAME,
	$workableFormPostId,
	[
		'formDataTypeSelector' => $workableFormDataTypeSelector,
		'ssr' => $workableFormServerSideRender,
	]
);
