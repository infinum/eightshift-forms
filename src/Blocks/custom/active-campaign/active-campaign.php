...
$activeCampaignFormServerSideRender = Components::checkAttr('activeCampaignFormServerSideRender', $attributes, $manifest);
$activeCampaignFormPostId = Components::checkAttr('activeCampaignFormPostId', $attributes, $manifest);
$activeCampaignFormDataTypeSelector = Components::checkAttr('activeCampaignFormDataTypeSelector', $attributes, $manifest);

// Check if activeCampaign data is set and valid.
$isSettingsValid = apply_filters(
	SettingsActiveCampaign::FILTER_SETTINGS_IS_VALID_NAME,
	$activeCampaignFormPostId
);

$activeCampaignClass = Components::classnames([
	Components::selector($blockClass, $blockClass),
	Components::selector(!$isSettingsValid, $invalidClass),
]);

// Bailout if settings are not ok but show msg only in editor.
if (!$isSettingsValid && $activeCampaignFormServerSideRender) {
	?>
		<div class="<?php echo esc_attr($activeCampaignClass); ?>">
			...
			<br />
			<b><?php esc_html_e('The ActiveCampaign integration is not configured correctly.', 'eightshift-forms'); ?></b>
			<br />
