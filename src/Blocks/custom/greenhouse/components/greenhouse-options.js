...
export const GreenhouseOptions = ({ attributes, setAttributes, postId }) => {
	const {
		settingsPageUrl,
	} = select(STORE_NAME).getSettings();

	const greenhouseIntegrationId = checkAttr('greenhouseIntegrationId', attributes, manifest);
	...
	useEffect( () => {
		apiFetch({ path: 'eightshift-forms/v1/integration-items-greenhouse' }).then((response) => {
	...
	return (
		<PanelBody title={__('Greenhouse', 'eightshift-forms')}>
			...
			<CustomSelect
				...
				value={greenhouseIntegrationId}
				options={formData}
				onChange={(value) => setAttributes({ [getAttrKey('greenhouseIntegrationId', attributes, manifest)]: value.toString() })}
				isClearable={false}
				cacheOptions={false}
