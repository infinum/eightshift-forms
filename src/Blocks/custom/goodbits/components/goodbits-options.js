...
export const GoodbitsOptions = ({ attributes, setAttributes, postId }) => {
	const {
		settingsPageUrl,
	} = select(STORE_NAME).getSettings();

	const goodbitsIntegrationId = checkAttr('goodbitsIntegrationId', attributes, manifest);
	...
	useEffect( () => {
		apiFetch({ path: 'eightshift-forms/v1/integration-items-goodbits' }).then((response) => {
	...
	return (
		<PanelBody title={__('Goodbits', 'eightshift-forms')}>
			...
			<CustomSelect
				...
				value={goodbitsIntegrationId}
				options={formData}
				onChange={(value) => setAttributes({ [getAttrKey('goodbitsIntegrationId', attributes, manifest)]: value.toString() })}
				isClearable={false}
				cacheOptions={false}
