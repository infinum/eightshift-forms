...
export const hooks = () => {
	const { blockName } = select(STORE_NAME).getBlock('mailchimp');
		...
		if (name === `${namespace}/${blockName}`) {
			if (typeof esFormsLocalization !== 'undefined' && isArray(esFormsLocalization?.additionalBlocks)) {
				esFormsLocalization.additionalBlocks.forEach((element) => {
					if (!settings.attributes.mailchimpAllowedBlocks.default.includes(element)) {
						settings.attributes.mailchimpAllowedBlocks.default.push(element);
					}
				});
			}
