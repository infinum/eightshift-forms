...
export const hooks = () => {
	const { blockName } = select(STORE_NAME).getBlock('corvus');
		...
		if (name === `${namespace}/${blockName}`) {
			if (typeof esFormsLocalization !== 'undefined' && isArray(esFormsLocalization?.additionalBlocks)) {
				esFormsLocalization.additionalBlocks.forEach((element) => {
					if (!settings.attributes.corvusAllowedBlocks.default.includes(element)) {
						settings.attributes.corvusAllowedBlocks.default.push(element);
					}
				});
			}
