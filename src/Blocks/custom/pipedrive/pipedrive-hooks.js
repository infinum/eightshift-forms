...
export const hooks = () => {
	const { blockName } = select(STORE_NAME).getBlock('pipedrive');
		...
		if (name === `${namespace}/${blockName}`) {
			if (typeof esFormsLocalization !== 'undefined' && isArray(esFormsLocalization?.additionalBlocks)) {
				esFormsLocalization.additionalBlocks.forEach((element) => {
					if (!settings.attributes.pipedriveAllowedBlocks.default.includes(element)) {
						settings.attributes.pipedriveAllowedBlocks.default.push(element);
					}
				});
			}
