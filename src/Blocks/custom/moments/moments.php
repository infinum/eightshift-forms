...
$momentsFormServerSideRender = Components::checkAttr('momentsFormServerSideRender', $attributes, $manifest);
$momentsFormPostId = Components::checkAttr('momentsFormPostId', $attributes, $manifest);
$momentsFormDataTypeSelector = Components::checkAttr('momentsFormDataTypeSelector', $attributes, $manifest);

// Check if moments data is set and valid.
$isSettingsValid = apply_filters(
	SettingsMoments::FILTER_SETTINGS_IS_VALID_NAME,
	$momentsFormPostId
);

$momentsClass = Components::classnames([
	Components::selector($blockClass, $blockClass),
	Components::selector(!$isSettingsValid, $invalidClass),
]);

// Bailout if settings are not ok but show msg only in editor.
if (!$isSettingsValid && $momentsFormServerSideRender) {
	?>
		<div class="<?php echo esc_attr($momentsClass); ?>">
			...
			<br />
			<b><?php esc_html_e('The Moments integration is not configured correctly.', 'eightshift-forms'); ?></b>
			<br />
