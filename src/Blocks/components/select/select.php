...
$shouldPrefill = isset($attributes['prefillData']) && filter_var($attributes['prefillData'], FILTER_VALIDATE_BOOLEAN);
$hideLoading = isset($attributes['hideLoading']) ?
	filter_var($attributes['hideLoading'], FILTER_VALIDATE_BOOLEAN) :
	true;
...
<div class="<?php
echo \esc_attr($componentClasses); ?>">
	<?php
	echo wp_kses_post(
		Components::render(
			'label',
	...
	?>
	<div class="<?php
	echo \esc_attr($contentWrapClasses); ?>">
		<select <?php !empty($selectId) ? printf('id="%s"', \esc_attr($selectId)) : ''; // phpcs:ignore Eightshift.Security.CustomEscapeOutput.OutputNotEscaped ?>
