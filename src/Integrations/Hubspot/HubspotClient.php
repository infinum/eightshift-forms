...
class HubspotClient implements HubspotClientInterface
{
	...
	public function postApplication(array $formDetails): array
	{
		...
		$filterName = HooksHelpers::getFilterName(['integrations', SettingsHubspot::SETTINGS_TYPE_KEY, 'overridePostRequest']);
		...
		$filterName = HooksHelpers::getFilterName(['integrations', SettingsHubspot::SETTINGS_TYPE_KEY, 'prePostId']);
		if (\has_filter($filterName)) {
	...
	{
		$folder = SettingsHelpers::getSettingValue(SettingsHubspot::SETTINGS_HUBSPOT_FILEMANAGER_FOLDER_KEY, $formId);
	...
	private function getFieldsErrors(array $body): array
	{
		...
		foreach ($msg as $value) {
			...
			if ($key === 'REQUIRED_FIELD') {
				// Validate req fields.
				\preg_match_all("/(Required field) '(\w+)' (is missing)/", (string) $message, $matchesReq, \PREG_SET_ORDER, 0);
	...
	private function prepareConsent(array $params, string $itemId): array
	{
		...
		foreach ($params as $param) {
			...
			$value = $param['value'] ?? '';
			$name = $param['name'] ? \explode(Config::DELIMITER, (string) $param['name']) : [];
				...
				if ($typeCustom === self::HUBSPOT_CONSENT_COMMUNICATION) {
					$communicationOutput[] = [
						...
						'text' => \array_values(\array_filter(
							...
							static fn(array $item): bool => $item['id'] === \end($name)
						))[0]['label'],
					];
				}
			}
	...
	private function prepareParams(array $params): array
	{
		...
		foreach ($params as $param) {
			...
			if ($typeCustom === self::HUBSPOT_CONSENT_LEGITIMATE) {
				continue;
			}
			if ($typeCustom === self::HUBSPOT_CONSENT_PROCESSING) {
				continue;
	...
	{
		return SettingsHelpers::getOptionWithConstant(Variables::getApiKeyHubspot(), SettingsHubspot::SETTINGS_HUBSPOT_API_KEY_KEY);
	}
