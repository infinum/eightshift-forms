...
class SettingsFriendlyCaptcha implements SettingGlobalInterface, ServiceInterface
{
	...
	{
		$isUsed = SettingsHelpers::isOptionCheckboxChecked(self::SETTINGS_FRIENDLY_CAPTCHA_USE_KEY, self::SETTINGS_FRIENDLY_CAPTCHA_USE_KEY);
		...
		if (!$isUsed || !$siteKey || !$apiKey) {
			return false;
		}

		// Mutual exclusivity: if Google reCAPTCHA is also valid, Friendly Captcha is not valid.
	...
	public function getSettingsGlobalData(): array
	{
		...
		if (!SettingsHelpers::isOptionCheckboxChecked(self::SETTINGS_FRIENDLY_CAPTCHA_USE_KEY, self::SETTINGS_FRIENDLY_CAPTCHA_USE_KEY)) {
			return SettingsOutputHelpers::getNoActiveFeature();
		}

		// Show warning if Google reCAPTCHA is also active.
		if (SettingsHelpers::isOptionCheckboxChecked(SettingsCaptcha::SETTINGS_CAPTCHA_USE_KEY, SettingsCaptcha::SETTINGS_CAPTCHA_USE_KEY)) {
			return [
				...
				[
					...
					'highlightedContentTitle' => \__('Google reCAPTCHA is active', 'eightshift-forms'),
					// phpcs:ignore WordPress.WP.I18n.NoHtmlWrappedStrings
					'highlightedContentSubtitle' => \__('Please disable Google reCAPTCHA before configuring Friendly Captcha.<br />Only one spam prevention provider can be active at a time.', 'eightshift-forms'),
					'highlightedContentIcon' => 'warning',
				],
			];
		}
