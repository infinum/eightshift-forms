...
class Security implements SecurityInterface
{
	...
	public function isRequestValid(): bool
	{
		...
		$keyName = SettingsHelper::getOptionName($key);
		$data = SettingsHelper::getOptionValueGroup($key);
		...
		if (($time - $timestamp) > \intval(SettingsHelper::getOptionValueWithFallback(SettingsSecurity::SETTINGS_SECURITY_RATE_LIMIT_WINDOW_KEY, (string) self::RATE_LIMIT_WINDOW))) {
			unset($data[$ip]);
		...
		if ($count >= \intval(SettingsHelper::getOptionValueWithFallback(SettingsSecurity::SETTINGS_SECURITY_RATE_LIMIT_KEY, (string) self::RATE_LIMIT))) {
			return false;
		}
