...
class RateLimitingLogEntry
{
	...
	public function write()
	{
		...
		global $wpdb;
		$identifier = "{$wpdb->prefix}{$tableName}";

		$wpdb->query($wpdb->prepare("INSERT INTO %i (form_id, user_token, activity_type, created_at) VALUES (%d, %s, %s, %d)", $identifier, $this->formId, $this->userToken, $this->activityType, $createdAt));
	...
	public static function find(
		...
		$results = $wpdb->get_results($wpdb->prepare("SELECT * FROM %i WHERE user_token = %s AND activity_type = %s AND created_at >= %d", $identifier, $userToken, $activityType, $windowStart));
	...
	public static function countByFormId(
		...
		return (int)($wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM %i WHERE user_token = %s AND form_id = %d AND created_at >= %d", $identifier, $userToken, $formId, $windowStart)));
	}
	...
	public static function findAggregatedByActivityType(
		...
		$results = $wpdb->get_results($wpdb->prepare(
			"SELECT activity_type, COUNT(*) as count FROM %i WHERE user_token = %s AND created_at >= %d GROUP BY activity_type",
			$identifier,
			$userToken,
			$windowStart
		));
	...
	public static function cleanup(int $windowDuration)
	{
		...
		$wpdb->query($wpdb->prepare("DELETE FROM %i WHERE created_at < %d", $identifier, $windowStart));
	}
