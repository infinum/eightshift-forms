...
function getTestTypeUnit(): string {
	return 'unit';
}
...
function setupMocks() {
	...
	Functions\when('get_transient')->alias(function (string $transient) {
		switch ($transient) {
			case 'array':
				return ['array'];
			case 'boolTrue':
	...
	Functions\when('wp_remote_get')->alias(function (string $url, array $args) {
		...
		switch ($url) {
			case 'https://success.test':
				$output['body'] = wp_json_encode(['array']);
				$output['response'] = [
					'code' => 200,
					'message' => 'OK',
				];
			case 'https://error.test':
				$output['body'] = wp_json_encode(['array']);
				$output['response'] = [
					'code' => 400,
					'message' => 'ERROR',
	...
	Functions\when('current_datetime')->justReturn('time');
