...
test('should show validation message for input text if number of characters is greater than expected', async ({ page }) => {
	...
	test('should show validation message for input number if value contains anything but numbers', async ({ page }) => {
		await openUrl(page, 'validation');
		await setTestEnvironment(page);
		await waitFormLoaded(page);
		await populateInput(page, 'input-number', 'asdf1234');
		await submitFormAction(page);
		await testFieldValidationMessage(page, 'input-number', 'This field should only contain numbers.');
	});
});
