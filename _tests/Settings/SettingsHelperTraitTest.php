...
use EightshiftFormsUtils\Helpers\UtilsSettingsHelper;
...
test('getSettingValue returns the correct post meta value', function() {
	putenv('test_force_post_meta_es-forms-test-HR=test');

	expect(UtilsSettingsHelper::getSettingValue('test', 1))->toBe('test');

	putenv('test_force_post_meta_es-forms-test-HR');
});

test('getSettingValueGroup returns the correct post meta value', function() {
	putenv('test_force_post_meta_es-forms-test-HR=[1]');

	expect(UtilsSettingsHelper::getSettingValueGroup('test', 1))->toBe([1]);
...
test('getOptionValue returns the correct option value', function() {
	...
	expect(UtilsSettingsHelper::getOptionValue('test', 1))->toBe('test');
...
test('getOptionValueGroup returns the correct option value', function() {
	...
	expect(UtilsSettingsHelper::getOptionValueGroup('test', 1))->toBe([1]);
...
test('getOptionCheckboxValues returns the correct option value', function() {
	...
	expect(UtilsSettingsHelper::getOptionCheckboxValues('test', 1))->toBe(['test', 'best', 'rest']);
...
test('isCheckedSettings returns correctly when post meta matches key', function($key, $id, $formId, $return) {
	...
	expect(UtilsSettingsHelper::isCheckedSettings($key, $id, $formId))->toBe($return);
...
// 	expect(UtilsSettingsHelper::isCheckedSettings((string)$key, $id, $formId))->toBe($return);
...
// test('getSettingValueGroup returns an empty array when an option isn\'t set', function() {
// 	putenv("test_force_post_meta_es-forms-nonexistent-HR=bool_false");

// 	expect(UtilsSettingsHelper::getSettingValueGroup('nonexistent', 1))->toBe([]);
...
// 	expect(UtilsSettingsHelper::getOptionValueGroup('nonexistent'))->toBe([]);
...
// 	expect(UtilsSettingsHelper::getOptionCheckboxValues('nonexistent'))->toBe([]);

// 	putenv("test_force_option_es-forms-nonexistent-HR");
// });

// test('isOptionChecked returns correctly when option doesn\'t match key', function($key, $id, $formId, $return) {
// 	putenv("test_force_option_es-forms-{$id}-HR=notreally{$key}");

// 	expect(UtilsSettingsHelper::isOptionChecked($key, $id))->toBe($return);
...
// test('isOptionChecked returns correctly when option matches key', function($key, $id, $formId, $return) {
...
//  	expect(UtilsSettingsHelper::getLocale())->toBe('HRR');
...
// 	expect(UtilsSettingsHelper::getIntegrationFieldsDetails($key, $type, $formFields, $formId, $additionalLabel))->toBe($return);
